...
class TestMattermostMultiImage:
    ...
    def test_local_file_metadata_sets_resolved_root_id(self, adapter, tmp_path):
        """Thread metadata is preserved for single-file Mattermost posts."""
        p = tmp_path / "img.png"
        p.write_bytes(b"\x89PNG" + b"\x00" * 20)

        _run(adapter.send_image_file("channel123", str(p), metadata={"thread_id": "reply123"}))
