...
def _codex_tool_descriptor(item: dict) -> Optional[Tuple[str, str, dict]]:
    """Return (call_id, tool_name, args) for a tool-shaped codex item."""
...
def _codex_tool_result(item: dict) -> str:
    """Return a display result for a completed tool-shaped codex item."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def _codex_live_event(agent, note: dict) -> None:
    """Bridge codex app-server notifications to live display callbacks."""
    try:
        ...
        if method in ("item/started", "item/completed"):
            ...
            if method == "item/started":
                ...
                if cb is not None:
                    cb(call_id, name, args, _codex_tool_result(item))
    except Exception:
