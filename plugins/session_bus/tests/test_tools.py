"""Offline tests for the bus_establish tool handler.
...
from plugins.session_bus import tools as _tools
...
class EstablishHarness(unittest.TestCase):
    ...
    def _call(self, resolve=_fake_resolve_target, **kwargs):
        ...
        with patch.dict("os.environ", ENV), \
             patch("plugins.session_bus.tools.routing.NATSRoutingClient", FakeClient), \
             patch("plugins.session_bus.nats_client.NATSRoutingClient", FakeClient), \
             patch.object(_presence, "resolve_target", resolve):
            return _tools.handle_bus_establish(**kwargs)
...
class FailurePaths(EstablishHarness):
    ...
    def test_no_session_env(self):
        ...
        with patch.dict("os.environ", env, clear=False) as patched:
            ...
            result = _tools.handle_bus_establish(target=PEER_ADDR)
...
class OutboundNotifications(unittest.TestCase):
    ...
    def test_route_send_message_text_notifies_sender_session(self):
        ...
        with p1, p2, p3, p4, p5:
            result = _tools.handle_bus_route_send(
    ...
    def test_establish_initial_message_notifies(self):
        p1, p2, p3, p4, p5 = self._patches()
        with p1, p2, p3, p4, p5:
            result = _tools.handle_bus_establish(
                target=PEER_ADDR,
...
class SessionHandleShareText(unittest.TestCase):
    ...
    def test_share_text_matches_expected_format(self):
        with patch.dict("os.environ", ENV):
            result = _tools.handle_bus_handle()
        self.assertTrue(result["ok"], result)
        self.assertEqual(
            result["share_text"],
...
class SchemaSurface(unittest.TestCase):
    def test_schema_shape(self):
        schema = _tools.ESTABLISH_SCHEMA
        self.assertEqual(schema["name"], "bus_establish")
    ...
    def test_registered_in_plugin_tools(self):
        import plugins.session_bus as plugin
        from unittest.mock import MagicMock
        # Run the plugin's register() against a stub ctx, then inspect
        # the populated _TOOLS list. This is the same pattern the legacy
        # session-bridge and session-routing plugins used.
        ctx = MagicMock()
        plugin._TOOLS.clear()
        plugin.register(ctx)
        names = [t[0] for t in plugin._TOOLS]
        self.assertIn("bus_emit", names)
        self.assertIn("bus_observe", names)
        self.assertIn("bus_handle", names)
        self.assertIn("bus_route_send", names)
        self.assertIn("bus_route_list", names)
        self.assertIn("bus_establish", names)
        # All 6 tools must register through ctx.register_tool
        self.assertEqual(ctx.register_tool.call_count, 6)
        # 3 hooks must register through ctx.register_hook
        self.assertEqual(ctx.register_hook.call_count, 3)


class SendSidePayloadValidation(OutboundNotifications):
    """bus_route_send fails closed on payloads the recipient's
    ...
    def _send(self, content):
        p1, p2, p3, p4, p5 = self._patches()
        with p1, p2, p3, p4, p5:
            return _tools.handle_bus_route_send(
    ...
    def test_omitted_content_rejected(self):
        p1, p2, p3, p4, p5 = self._patches()
        with p1, p2, p3, p4, p5:
            result = _tools.handle_bus_route_send(target=PEER_ADDR)
        self.assertFalse(result["ok"])
        self.assertEqual(result["error"], "invalid_payload")
