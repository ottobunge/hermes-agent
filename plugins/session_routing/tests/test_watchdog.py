...
from plugins.session_routing.runtime import SessionRoutingRuntime
...
class InboxRunnerActivityIntegration(unittest.TestCase):
    ...
    def test_empty_fetches_stamp_activity_clock(self):
        async def _go():
            ...
            with patch(
                "plugins.session_routing.inbox.NATSRoutingClient", _FakeClient
            ), patch(
                "plugins.session_routing.inbox.effective_allow_list", _allow
            ):
                runner.start()
    ...
    def test_dispatch_count_increments_on_successful_callback(self):
        ...
        async def _go():
            ...
            client.remaining = 3
            with patch(
                "plugins.session_routing.inbox.NATSRoutingClient",
                lambda *a, **k: client,
            ), patch(
                "plugins.session_routing.inbox.effective_allow_list", _allow
            ):
...
class WatchdogRestart(unittest.TestCase):
    ...
    def test_watchdog_respawns_when_runner_silently_dies(self):
        ...
        import plugins.session_routing.runtime as _rt_mod
...
class HealthThreshold(unittest.TestCase):
    ...
    def test_threshold_is_exported(self):
        from plugins.session_routing.inbox import DEFAULT_HEALTH_THRESHOLD_SECONDS
